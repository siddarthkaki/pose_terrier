...
class MeasResidCostFunctorQuat
{
                    ...
                    T* residuals) const
    {
        ...
        for (int idx = 0; idx < numPts; idx++)
        {
            ...
            residuals[2*idx+0] = 1.0/sqrt(bearing_meas_std_) * ( yVec_(2*idx+0) - static_cast<T>(atan2( rFeaVeciRotatedTranslated(0), rFeaVeciRotatedTranslated(2) )) );
            residuals[2*idx+1] = 1.0/sqrt(bearing_meas_std_) * ( yVec_(2*idx+1) - static_cast<T>(atan2( rFeaVeciRotatedTranslated(1), rFeaVeciRotatedTranslated(2) )) );
