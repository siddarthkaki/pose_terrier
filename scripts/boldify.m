...
for i=1:length(ha)
  if (strcmp(get(ha(i),'Type'),'axes') == 1)
    ...
    set(ha(i),'FontSize',font_size); 		% Tick mark and frame format
    ...
    set(get(ha(i),'XLabel'),'FontSize',font_size); % font_size-pt labels
    set(get(ha(i),'XLabel'),'FontWeight','Bold');
    set(get(ha(i),'XLabel'),'VerticalAlignment','top');
    set(get(ha(i),'YLabel'),'FontSize',font_size); % font_size-pt labels
    set(get(ha(i),'YLabel'),'FontWeight','Bold');
    set(get(ha(i),'YLabel'),'VerticalAlignment','baseline');
    set(get(ha(i),'Title'),'FontSize',20); % 16-pt titles
  ...
  for j=1:length(hc)
    ...
    if chtype(1:4) == 'text'
      set(hc(j),'FontSize',font_size);		% font_size-pt descriptive labels
